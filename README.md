...
| 接口 | 优化前 QPS | 优化后 QPS | 最优并发 | 错误率 |
|------|-----------|-----------|----------|--------|
| GetUploadStatus | 8769 | 9964 | 100/500 | 0% |
| UploadChunk | 5594 | 5368 | 1000 | 0% |
| InitUpload | 4328 | **5446** | 100 | 0% |
| CompleteUpload | 842 | 945 | 30 | 0% |

- 全部 16 个测试用例（并发 5~2000）**零错误**
- Redis Sentinel failover 后服务自动恢复，性能无损
- 最优并发区间：**500-1000**
- **优化**：InitUpload 用 Redis Pipeline 合并 3 RTT → 1 RTT，QPS +26%（4328→5446）
