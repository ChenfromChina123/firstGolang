...
- **安全加固**：基于 aistudy.icu 安全评估报告（评分 7.3/10）实施加固：①8 个安全响应头（HSTS/CSP/Permissions-Policy/X-Frame-Options/X-Content-Type-Options/Referrer-Policy/X-Permitted-Cross-Domain-Policies/X-XSS-Protection）；②HTTP 方法限制中间件（拒绝 TRACE/CONNECT，返回 405）；③分享端点速率限制（60 次/分钟/IP，防分享 ID 暴力枚举）；④登录限流收紧（burst 从 5 降为 3，共 8 次/分钟）；⑤security.txt 安全联系信息文件。中间件链：SecurityHeaders -> MethodGuard -> RefererCheck -> JWT -> mux
