...
# 上传 JSON 文件（使用 curl -F 上传）
print('\n=== 2. 上传 JSON 测试文件 ===')
# 先 init
init_result = run(f"""curl -s -b /tmp/cookies_test.txt -X POST https://aistudy.icu/api/upload/init -H 'Content-Type: application/json' -d '{{"filename":"test_preview.json","file_size":{os.path.getsize(json_path)},"chunk_size":1048576,"storage":"local"}}'""")
print(f'Init: {init_result}')
try:
    init_data = json.loads(init_result)
    session_id = init_data.get('session_id')
    total_chunks = init_data.get('total_chunks', 1)
    print(f'Session: {session_id}, Total chunks: {total_chunks}')

    # upload chunk
    upload_result = run(f"""curl -s -b /tmp/cookies_test.txt -X POST https://aistudy.icu/api/upload/chunk -F 'session_id={session_id}' -F 'chunk_index=0' -F 'chunk_data=@{json_path}'""")
...
# 上传 CSV 文件
print('\n=== 3. 上传 CSV 测试文件 ===')
init_result2 = run(f"""curl -s -b /tmp/cookies_test.txt -X POST https://aistudy.icu/api/upload/init -H 'Content-Type: application/json' -d '{{"filename":"test_preview.csv","file_size":{os.path.getsize(csv_path)},"chunk_size":1048576,"storage":"local"}}'""")
print(f'Init: {init_result2}')
try:
    init_data2 = json.loads(init_result2)
    session_id2 = init_data2.get('session_id')

    upload_result2 = run(f"""curl -s -b /tmp/cookies_test.txt -X POST https://aistudy.icu/api/upload/chunk -F 'session_id={session_id2}' -F 'chunk_index=0' -F 'chunk_data=@{csv_path}'""")
...
if json_file_id:
    ...
    print(f'JSON content: {json_content_resp[:200]}')
...
if csv_file_id:
    ...
    print(f'CSV content: {csv_content_resp[:200]}')
